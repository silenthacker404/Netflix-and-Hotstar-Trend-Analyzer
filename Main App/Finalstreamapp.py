...
if uploaded_file:
    try:
        ...
        chart_type = st.radio("Select chart type", ["Bar Chart", "Pie Chart", "Scatter Plot"])
        ...
        if df[selected_col].dtype == "object":
            ...
            chart_df = df[selected_col].dropna().value_counts().reset_index()
            chart_df.columns = ['value', 'count']
        ...
        if chart_type == "Bar Chart":
            ...
            if df[selected_col].dtype != 'object':
                fig = px.scatter(df, x=selected_col, y=df.columns[-1],
                                 title=f"Scatter Plot: {selected_col} vs {df.columns[-1]}",
                                 labels={selected_col: selected_col.title(), df.columns[-1]: df.columns[-1].title()})
            else:
                st.warning("Scatter plot is not suitable for non-numeric columns.")
                fig = None
